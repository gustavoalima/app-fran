...
const AvaliacaoMasculina = () => {
  ...
  const salvarAvaliacao = async () => {
    try {
      ...
      const response = await axios.post('http://localhost:5000/avaliacao-masculina', {
        aluno_id: alunoId,
        nome,
        idade,
        peso,
        ...
        percentualMassaMagra: resultados.percentualMassaMagra,
        massaGorduraKg: resultados.massaGorduraKg,
        massaMagraKg: resultados.massaMagraKg,
        imc: resultados.imc,
  ...
  useEffect(() => {
    const fetchAlunoData = async () => {
      try {
        const response = await axios.get(`http://localhost:5000/alunos/${alunoId}`);
